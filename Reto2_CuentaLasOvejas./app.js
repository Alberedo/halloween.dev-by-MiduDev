...
function countSheep(letters) {
  ...
  for(let elem of letters){
    ltrs.includes(elem) ? !count[elem] ? count[elem]=1 : count[elem]++ : null
  }
  if(!count["s"] || !count["h"] || !count["e"] || !count["p"] ){
    return 0
  }else{
    count["e"] = count["e"]/2
    let mini = parseInt(Math.min(...Object.values(count)))
    return (Math.max(mini));
