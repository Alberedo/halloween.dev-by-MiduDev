...
let aux3 = 0
    if(current === target){
        return 0
          ...
          console.log(currentArr, targetArr)
          for(let i=0; i< targetArr.length; i++){
            currentNumber = Number.parseInt(currentArr[i])
            targetNumber = Number.parseInt(targetArr[i])
              if(currentNumber==targetNumber){
                  moves = moves + 0
              } else{
                  aux1 = targetNumber - currentNumber           
                  aux2 = currentNumber + 10 - targetNumber
                  aux3 = targetNumber + 10 - currentNumber
                  moves += Math.min(Math.abs(aux1), Math.abs(aux2), Math.abs(aux3))
              }
          }
    }
  return moves
